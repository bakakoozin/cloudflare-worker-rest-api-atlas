...
// .wrangler/tmp/bundle-Vs1si5/checked-fetch.js
...
async function getClient(env, App3) {
  try {
    const credentials = Credentials.apiKey(env.ATLAS_TOKEN);
    const user = await App3.logIn(credentials);
...
// src/ctrl/derbyname.ctrl.ts
var DERBYNAMES = "todos";
var derbyNameController = {
  async GET(req, env, App3) {
    const client = await getClient(env, App3);
    if (!client)
      return toError("probl\xE8me connexion MdB", 500);
    const collection = client.db(DB_NAME).collection(DERBYNAMES);
    const derbyNames = await collection.find({});
    return toJSON(derbyNames.map((d) => ({
      name: d.name,
      numRoster: d.numRoster
    })));
  },
  async POST(req, env, App3) {
...
var worker = {
  async fetch(req, env) {
    ...
    App2 = App2 || new App(env.ATLAS_APPID);
    const method = req.method;
    const path = url.pathname.replace(/[/]$/, "");
    const todoID = url.searchParams.get("id") || "";
    const router = { "/derbynames": derbyNameController };
    const controller = router?.[path]?.[method];
    return typeof controller === "function" ? await controller(req, env, App2) : toError("not found", 404);
  }
};
...
// .wrangler/tmp/bundle-Vs1si5/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Vs1si5/middleware-loader.entry.ts
